...
esp_err_t comm_uart_init(void) {
  ...
  ret = uart_set_pin(UART_NUM, UART_TX_PIN, UART_RX_PIN, UART_PIN_NO_CHANGE,
                     UART_PIN_NO_CHANGE);
  if (ret != ESP_OK) {
  ...
  ESP_LOGI(TAG, "UART initialized on port %d (TX: %d, RX: %d)", UART_NUM,
...
comm_result_t comm_uart_send_command(const char *command, char *response_buf,
  ...
  // Wait for response
  int received = uart_read_bytes(UART_NUM, (uint8_t *)response_buf,
                                 buf_size - 1, pdMS_TO_TICKS(UART_TIMEOUT_MS));
  if (received < 0) {
...
esp_err_t comm_i2c_write(uint8_t device_addr, uint8_t reg_addr, uint8_t data) {
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGD(TAG, "I2C write: addr=0x%02X, reg=0x%02X, data=0x%02X",
...
esp_err_t comm_i2c_read(uint8_t device_addr, uint8_t reg, uint8_t *data,
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGD(TAG, "I2C read: addr=0x%02X, reg=0x%02X, size=%d", device_addr,
             reg, size);
  }
