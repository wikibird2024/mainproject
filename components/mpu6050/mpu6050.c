...
/// ======== Định nghĩa & Macro ========
#define TAG "MPU6050"                          // Tag cho ESP_LOG
#define I2C_MASTER_NUM I2C_NUM_0               // Bus I2C sử dụng
#define COMBINE_BYTES(msb, lsb) ((int16_t)(((msb) << 8) | (lsb))) // Ghép 2 byte thành int16_t

static const float ACCEL_SCALE_FACTOR = 16384.0f;   // Quy đổi accel raw -> g
static const float GYRO_SCALE_FACTOR = 131.0f;      // Quy đổi gyro raw -> độ/giây
static const float FALL_THRESHOLD = 0.7f;           // Ngưỡng phát hiện té ngã (g)
...
esp_err_t mpu6050_init(void) {
    ...
    // Gửi 0x00 đến PWR_MGMT_1 để đánh thức MPU6050 (thoát sleep mode)
    esp_err_t err = i2c_write(MPU6050_ADDR, MPU6050_PWR_MGMT_1, 0x00);
    if (err != ESP_OK) {
...
esp_err_t mpu6050_read_data(sensor_data_t *data) {
    ...
    uint8_t raw[14]; // 14 byte: accel(6), temp(2), gyro(6)
...
bool detect_fall(sensor_data_t data) {
    ...
    // Sai lệch so với trọng lực 1g
    float acc_delta = fabsf(acc_magnitude - 1.0f);

    ESP_LOGI(TAG, "Accel magnitude: %.2f g, Delta: %.2f g", acc_magnitude, acc_delta);
